...
b = model.addVars(12, vtype=GRB.BINARY, name="pp")
...
for t in range(11):
    model.addConstr(p[t] + b[t] >= p[t+1])
    model.addConstr(p[0] == b[0])
    model.addConstr(gp.quicksum(b[t] for t in range(12)) == 1)
